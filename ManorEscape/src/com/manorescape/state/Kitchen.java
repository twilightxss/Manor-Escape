...
public class Kitchen extends LevelState
{
    ...
    private final int xCoords[] = {100, 175, 230, 100, 175, 230};
    private final int yCoords[] = {100, 100, 100, 450, 450, 450};
    private final String furnitureType[] = {"table", "table" , "table", "table", "table", "table"};
    
    private TileMapManager tmm;
    private TileMap tm;
    
    public Kitchen(LevelManager lm)
    {
        super(lm);
    ...
    public void init()
    {
        try
        {
            background = ImageIO.read(getClass().getResourceAsStream("/images/kitchenbg.png"));
        }catch(Exception EX)
            {
                System.out.println("Error loading level 2 background image");
            }
        
        System.out.println("Kitchen loaded");
    }
